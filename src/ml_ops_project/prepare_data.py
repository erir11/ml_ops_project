...
def download_dataset(raw_path: Path) -> None:
    ...
    kaggle_path = kagglehub.dataset_download("sharko294/image-dataset-for-car-damage-classification")
...
def create_splits(
    ...
    train_df, temp_df = train_test_split(
        df, train_size=splits["train"], random_state=42, stratify=df["label"] if "label" in df.columns else None
    )
    ...
    for split_name, split_df in split_dfs.items():
        _save_split(split_df, raw_path / "train" / "images", processed_path / split_name, split_name)
...
def _are_splits_present(path: Path) -> bool:
    ...
    return all(
        (path / split / f"{split}.csv").exists()
        and (path / split / "images").exists()
        and any((path / split / "images").iterdir())
        for split in splits
    )
...
def prepare_dataset(
    ...
    splits = {"train": train_size, "val": val_size, "test": 1 - train_size - val_size}
