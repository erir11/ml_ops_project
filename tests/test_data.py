...
from ml_ops_project.data import CarDamageDataset, CarDamageDataModule  # Adjust import path as needed
...
def corrupted_data(tmp_path):
    ...
    data = {
        "filename": ["good_image.jpg", "corrupted_image.jpg"],
        "label": [1, 2]
    }
    df = pd.DataFrame(data)
    df.to_csv(csv_path, index=False)
...
def missing_images_data(tmp_path):
    ...
    csv_path = data_dir / split / f"{split}.csv"
    data = {
        "filename": ["image_1.jpg", "image_missing.jpg"],
        "label": [1, 2]
    }
    df = pd.DataFrame(data)
    df.to_csv(csv_path, index=False)
...
def test_car_damage_data_module_setup(dummy_data):
    ...
    """
    datamodule = CarDamageDataModule(
        data_dir=dummy_data,
        batch_size=4,
        num_workers=2,
        image_size=(256, 256)
